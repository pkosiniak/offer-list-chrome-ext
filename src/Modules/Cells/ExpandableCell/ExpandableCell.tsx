...
import CancelButton from '../../InteractiveButtons/CancelButton';
...
export interface ExpandableCellProps extends Required<WidthType> {
   onOkClick: () => void,
   onCancelClick: () => void,
   zIndex: number,
   className?: string,
...
type ExpandableChildren = {
   children: (expandableState: ExpandableCellState) => ReactNode
};

type ExpandableCellPropsType = ExpandableCellProps
   & ExpandableChildren

const ExpandableCell: React.FC<ExpandableCellPropsType> = ({
   onOkClick,
   onCancelClick,
   width,
   zIndex,
